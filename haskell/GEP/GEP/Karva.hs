...
-- In GEP each gene represents a valid program. For the case of
-- parameter optimization (GEP-PO) problems each gene represents a
-- real valued parameter. In order to evaluate the fitness of each
-- individual it is necessary to execute (evaluate) each gene. This
-- amounts to constructing the parse tree and then evaluating it.
--
-- I had a lot of trouble trying to find a solution to build the parse
-- trees. The reason is that the list is built breadth-first. In
-- haskell it is natural to use recursion to unfold the tree
-- depth-first. It turns out the solution resides in the Data.Tree
-- module based of a paper on breadth-first tree unfolding.
